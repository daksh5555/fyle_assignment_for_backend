...
class AssignmentSubmitSchema(Schema):
    ...
    @post_load
    def initiate_class(self, data_dict, many, partial):
        # pylint: disable=unused-argument, and also no-self-use
...
""" 
 process:
because we 're creating a Schema for a table record. to do that we have to serialize the python obj 
"""

class TeacherSchema(SQLAlchemyAutoSchema):
   
    class Meta:
        #  what model to use to generate the auto schema.
        model = Teacher
        # exclude all them unknown pros and should not raise an error for that thing.
        unknown = EXCLUDE
    ...
    """  this method is ivoked when we got to deserialize. """
    @post_load
    def initiate_class(self, data_dict, many, partial):
        return GeneralObject(**data_dict)